export const FETCH_INGREDIENTS_SUCCESS: 'FETCH_INGREDIENTS_SUCCESS' = 'FETCH_INGREDIENTS_SUCCESS';
export const FETCH_INGREDIENTS_FAIL: 'FETCH_INGREDIENTS_FAIL' = 'FETCH_INGREDIENTS_FAIL';
export const ADD_SELECTED_BUN: 'ADD_SELECTED_BUN' = 'ADD_SELECTED_BUN';
export const REMOVE_SELECTED_BUN: 'REMOVE_SELECTED_BUN' = 'REMOVE_SELECTED_BUN';
export const ADD_SELECTED_INGREDIENT: 'ADD_SELECTED_INGREDIENT' = 'ADD_SELECTED_INGREDIENT';
export const ADD_SELECTED_INGREDIENTS: 'ADD_SELECTED_INGREDIENTS' = 'ADD_SELECTED_INGREDIENTS';
export const REMOVE_SELECTED_INGREDIENT: 'REMOVE_SELECTED_INGREDIENT' =
  'REMOVE_SELECTED_INGREDIENT';
export const REMOVE_SELECTED_INGREDIENTS: 'REMOVE_SELECTED_INGREDIENTS' =
  'REMOVE_SELECTED_INGREDIENTS';
export const ADD_DETAILED_INGREDIENT: 'ADD_DETAILED_INGREDIENT' = 'ADD_DETAILED_INGREDIENT';
export const REMOVE_DETAILED_INGREDIENT: 'REMOVE_DETAILED_INGREDIENT' =
  'REMOVE_DETAILED_INGREDIENT';
export const INCREMENT_INGREDIENT_COUNT: 'INCREMENT_INGREDIENT_COUNT' =
  'INCREMENT_INGREDIENT_COUNT';
export const DECREMENT_INGREDIENT_COUNT: 'DECREMENT_INGREDIENT_COUNT' =
  'DECREMENT_INGREDIENT_COUNT';
